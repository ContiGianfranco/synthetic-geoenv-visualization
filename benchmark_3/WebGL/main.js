...
async function mainNew() {
    ...
    let camara = new OrbilalCamara(v3.create(0, 0, 5));
    ...
    function animationLoop(now) {
        ...
        if (now !== 0 && now !== undefined)
        {
            if (!startFM)
            {
                ...
                const delta = now - lastframe; // en ms
                lastframe = now;

                if ((now - lastframe) > 1000) console.log("Delta (ms): ", now - lastframe, ", fpsCount:", fpsCount)

                fpsSum += 1000 / delta;
                fpsCount += 1;
                sumDelta += delta;

                let viewMatrix = camara.update(delta, input());
                let aspect = gl.canvas.clientWidth / gl.canvas.clientHeight;
                m4.perspective(1.309, aspect, 0.1, 2000, projectionMatrix);
                drawScene(viewMatrix, projectionMatrix);

                sumProsTime += performance.now() - startTime;

                if (Date.now() > (timer + 30000) && noStop) {
