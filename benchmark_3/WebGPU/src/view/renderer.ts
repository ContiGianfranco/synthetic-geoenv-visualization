...
export class Renderer {
   ...
   async Initialize() {
    ...
    async makePipeline() {
        ...
        this.pipeline = this.device.createRenderPipeline({
            vertex : {
                ...
                entryPoint : "vs_main",
                buffers: [this.quadMesh.bufferLayout, this.quadMesh.normalBufferLayout]
            },
    ...
    async render(renderables: RenderData) {
        ...
        mat4.perspective(projection, 1.309, this.canvas.width/this.canvas.height, 0.1, 100);
        ...
        const cam = new Float32Array(renderables.camara_pos);

        this.device.queue.writeBuffer(this.uniformBuffer, 0, <ArrayBuffer>view);
        this.device.queue.writeBuffer(this.uniformBuffer, 64, <ArrayBuffer>projection);
        this.device.queue.writeBuffer(this.uniformBuffer, 128, cam);
