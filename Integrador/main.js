...
// Para mediciones
let sumProsTime = 0, fpsCount = 0, fpsSum = 0, timer = 0, startFM = false, lastframe = 0, noStop = true;
let angle = 0;
const radius = 225;
...
async function init() {
    ...
    camera.position.set(150, 100, 75);
...
function tomarMediciones(elapsed) {
    if (elapsed !== 0 && elapsed !== undefined)
    {
        ...
        if (performance.now() > (30000 + timer) && noStop)
        {
